...
security_group_id=$(aws ec2 describe-security-groups --group-names "lanchonete-dev" --query "SecurityGroups[0].GroupId" --output text 2>/dev/null)

if [ -z "$security_group_id" ]; then
    echo ">[ERRO] Security group lanchonete-dev não foi criado na VPC $vpc_id"
    exit 1
fi
...
--tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=lanchonete-dev}]' \
